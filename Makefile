...
test.unit: test.setup ## run unit tests
	python -m pytest eventtracking --cov-report=html --cov-report term-missing  --cov-branch -k 'not integration and not performance' --cov-fail-under=95 --cov=eventtracking

test.integration: test.setup ## run integration tests
	python -m pytest --verbose -s -k 'integration'

test.performance: test.setup ## run performance tests
	python -m pytest --verbose -s -k 'performance'
